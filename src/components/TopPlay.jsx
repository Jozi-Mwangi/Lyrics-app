...
const TopChartCard = ({song, i, activeSong, handlePauseClick, handlePlayClick, isPlaying}) => (
  <div className="w-full flex flex-row items-center hover:bg-[#4c426e] py-3 p-4 rounded-lg cursor-pointer mb-2" >
    ...
    <div className="flex-1 flex-row justify-between items-center" >
      <img className="w-20 h-20 rounded-lg" src={song?.images} alt={song.track.name} /> 
      <div className="flex-1 flex flex-col justify-center mx-3" >
        <Link to={`/songs/${song.track}`} >
          <p className="text-xl font-bold text-white" >{song.track.name}</p>
        </Link>
        
        <Link to={`/artists/${song?.track.artists[0].name}`} >
          <p className="text-xl font-bold text-white" >{song.track.artists[0].name}</p>
        </Link>
      </div>
    </div>
    <PlayPause
          ...
          {topPlays.map((song,i ) => (
            ...
            >
              <Link to={`/artists/${song?.track.artists[0].name}`} >
                ...
                {/* <img src={song?.} className="rounded-full w-full object-cover" alt="name" /> */}
              </Link>
            </SwiperSlide>
          ))}
        </Swiper>
